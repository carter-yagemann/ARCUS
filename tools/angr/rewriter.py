...
"""
    ...
    def add_hook(self, addr, src):
        ...
        if isinstance(src, int):
            log.info("Adding hook at %#x to capture the value stored at %#x" % (addr, src))
        else:
        ...
        trampoline_size = 0
        for (address, size, mnemonic, op_str) in self.capstone.disasm_lite(raw_bytes, addr):
        ...
        self.hooks[addr] = {'src': src, 'orig_code': raw_ints[:trampoline_size],
                            'trampoline_size': trampoline_size, 'idx': None,
                            'section': section.name, 'offset': offset,
                            'src_sec': src_sec_name, 'src_off': src_off}
    ...
    def compile(self, new_bin):
        ...
        try:
            ...
            ret = os.WEXITSTATUS(os.system('gcc -Os -nostdlib -nodefaultlibs -fPIC -Wl,-shared "%s" -o "%s"' %
            ...
            for segment in hooks_so_section.segments:
                log.debug("Adding segment to be loaded at VA %#x" % self.elf.next_virtual_address)
            ...
            record_addr = record_sym.value - hooks_so_section.virtual_address + hooks_elf_section.virtual_address
            log.debug("record() function VA: %#x" % record_addr)
            hook_sym = hook_so.get_symbol('hook')
            hook_section_addr = hook_sym.value - hooks_so_section.virtual_address + hooks_elf_section.virtual_address
            ...
            for addr in self.hooks:
                ...
                section_va = self.elf.get_section(self.hooks[addr]['section']).virtual_address
                ...
                if len(trampoline) != self.jmp_size:
                    raise Exception("Unsupported trampoline instruction size: %d" % len(trampoline))
                ...
                if len(nop) != 1:
                    raise Exception("Expected nop to be 1 byte, instead it's %d" % len(nop))
                trampoline += (nop * (self.hooks[addr]['trampoline_size'] - len(trampoline)))
                ...
                instrs = [instr for instr in self.capstone.disasm(bytes(self.hooks[addr]['orig_code']), addr)]
                insn_offset = 0
                new_instrs = list()
                for insn in instrs:
                    new_asm = insn.mnemonic + ' ' + insn.op_str
                    for op in insn.operands:
                        if op.type == capstone.x86.X86_OP_MEM and op.value.mem.disp != 0:
                            log.debug("Fixing displacement in: %s" % new_asm)
                            new_disp = op.value.mem.disp + (new_addr - new_hook_addr + insn_offset)
                            new_asm = new_asm.replace("%#x" % op.value.mem.disp, "%#x" % new_disp)
                ...
                if num_insn != len(new_instrs):
                    raise Exception("Keystone only assembled %d out of %d instructions" % (num_insn, len(new_instrs)))
                ...
                if isinstance(hook_src, str):
                    if self.arch == lief.ELF.ARCH.x86_64:
                        ...
                        hook_bytes += self.keystone.asm("push %%rax; mov %%%s,%%rax" % hook_src)[0]
                        # the actual call and postamble
                        call_target = record_addr - (new_hook_addr + len(hook_bytes))
                        hook_bytes += self.keystone.asm("call %#x; pop %%rax" % call_target)[0]
                    else:
                        # preamble to calling record()
                        hook_bytes += self.keystone.asm("push %%eax; mov %%%s,%%eax" % hook_src)[0]
                        # the actual call and postamble
                        call_target = record_addr - (new_hook_addr + len(hook_bytes))
                        hook_bytes += self.keystone.asm("call %#x; pop %%eax" % call_target)[0]
                else:
                    if self.arch == lief.ELF.ARCH.x86_64:
                        # hook_src may be stale too
                        src_sec_va = self.elf.get_section(self.hooks[addr]['src_sec']).virtual_address
                        ...
                        mov_target = new_hook_src - (new_hook_addr + len(hook_bytes)) - 7
                        hook_bytes += self.keystone.asm("lea %#x(%%rip),%%rax" % mov_target)[0]
                        # the actual call and postamble
                        call_target = record_addr - (new_hook_addr + len(hook_bytes))
                        hook_bytes += self.keystone.asm("call %#x; pop %%rax" % call_target)[0]
                    else:
                        ...
                        # subtract 7 to account for instruction length
                        mov_target = new_hook_src - (new_hook_addr + len(hook_bytes)) - 7
                        hook_bytes += self.keystone.asm("lea %#x(%%eip),%%eax" % mov_target)[0]
                        # the actual call and postamble
                        call_target = record_addr - (new_hook_addr + len(hook_bytes))
                        hook_bytes += self.keystone.asm("call %#x; pop %%eax" % call_target)[0]
                ...
                jmp_back_target = new_addr + self.hooks[addr]['trampoline_size'] - jmp_back_site
...
def parse_args():
    ...
    parser.add_option('-l', '--log', action='store', type=int, default=20,
                      help='Set log level')
    parser.add_option('-p', '--ptwrite', action='store_true', default=False,
