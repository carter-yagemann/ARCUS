...
def parse_entry_state_json(project, trace_dir, snapshot_dir, prep_explore=False, override_max_argv=None):
        ...
        if keyword == 'argv':
            ...
            for env_item in state_json['env']:
                ...
                if prep_explore and env_item['key_type'] == 'BVS':
                    ...
                    env_key = claripy.BVV(env_item['key_val'], size=env_item['key_size'])
                ...
                if prep_explore and env_item['val_type'] == 'BVS':
                    ...
                    env_val = claripy.BVV(env_item['val_val'], size=env_item['val_size'])
    ...
    if not fs_files is None:
        if len(fs_files['files']) > 0 and 'BVS' in [arg['type'] for arg in state_json['argv']]:
            log.warning("The traced program appears to have received files via argv, but argv"
        ...
        for fp in fs_files['files']:
            ...
            simfile = angr.SimFile(fp, content=data, size=data_len, has_end=True, concrete=True)
...
def main():
    ...
    if xed.returncode != 0:
        log.warn("Disassembler returned non-zero code, analysis may not cover entire trace")
