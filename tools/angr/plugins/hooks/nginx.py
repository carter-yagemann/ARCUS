...
class ngx_cpystrn(angr.SimProcedure):
    ...
    def run(self, dst, src, n):
        return self.inline_call(angr.SIM_PROCEDURES['libc']['strncpy'], dst, src, n).ret_expr
...
class ngx_gmtime(angr.SimProcedure):
    ...
    def run(self, t, tp_ptr):
        store = lambda offset, val: self.state.memory.store(tp_ptr + offset, val, endness='Iend_LE')
        ...
        log.debug("ngx_gmtime: %s sec, %s min, %s hour, %s mday, %s mon, %s year, %s wday" %
                (str(sec), str(minute), str(hour), str(mday), str(mon), str(year), str(wday)))
