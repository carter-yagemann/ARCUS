...
class TestGriffinParser(unittest.TestCase):
    ...
    mem_layout = [
        {"filepath": "/usr/bin/last", "base_va": 0x55EA6C4FC000},
...
class TestPTCFG(unittest.TestCase):
    ...
    def setUp(self):
        if not os.path.isfile(self.test_bin) or not os.path.isfile(self.test_trace):
            self.skipTest(
                "Cannot find a PTCFG test file, are you in the correct directory?"
            )
        ...
        mem_info = [
            item for item in mem_info if os.path.basename(item["filepath"]) == bin_name
        ]
        assert len(mem_info) == 1
        p = Project(
            self.test_bin,
            load_options={"auto_load_libs": False},
            main_opts={"base_addr": mem_info[0]["base_va"]},
        )
...
class TestAnalysis(unittest.TestCase):
    ...
    def do_analysis_test(self, trace_name):
        ...
        for criteria in trace_info["reports"]:
            prefix_cnt = len(
                [name for name in reports if name.startswith(criteria["prefix"])]
            )
