...
PTXED = os.path.join(os.path.dirname(os.path.realpath(__file__)), '../pt/build/bin/ptxed')
IPT_SCRIPTS = os.path.join(os.path.dirname(os.path.realpath(__file__)), '../pt/libipt/script')
...
class Disasm(object):
    ...
    def __init__(self, perf_fp, aux_fp, sideband_fp):
        ...
        # get opts
        self.opts = subprocess.run(['/bin/bash', ipt_getopts, '-m', os.path.basename(self.sideband)],
...
def disasm_perf(perf_fp, output_fp):
    ...
    with gzip.open(output_fp, 'wt') as ofile:
        ...
        while True:
            ...
            timestamps = sorted(list(enumerate([w.time for w in workers])), key=lambda w: w[1])
            # record next event
