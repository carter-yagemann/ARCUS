...
class Tracer(ExplorationTechnique):
    ...
    def __init__(self, trace=None, start_address=None):
    ...
    def setup(self, simgr):
        ...
        if len(simgr.active) != 1:
            raise AngrTracerError(
                "Tracer is being invoked on a SimulationManager without exactly one active state"
            )
        ...
        if not found_entry:
            raise AngrTracerError(
                "Starting address isn't in trace, did you specify the correct PID?"
            )
        ...
        while self._trace and self._trace[idx] != simgr.one_active.addr:
            ...
            if len(simgr.active) == 0:
                raise AngrTracerError(
                    "Could not step to the first address of the trace - simgr is empty"
                )
            elif len(simgr.active) > 1:
                raise AngrTracerError(
                    "State split while searching for starting address"
                )
        ...
        if len(simgr.active) > 1:
            log.warn(
                "%d states reached the starting address, picking 1" % len(simgr.active)
            )
    ...
    def filter(self, simgr, state, **kwargs):
        ...
        if not state.solver.symbolic(state._ip) and state.arch.name != 'RISCV':
            ...
            if (
                ...
                and next_obj == state.project.loader.extern_object
            ):
                ...
                log.warning(
                    "Entering simulation procedure without a return frame, cannot continue"
                )
                return "traced"
    ...
    def step_state(self, simgr, state, **kwargs):
        ...
        if not None in succs_dict:
            raise AngrTracerError(
                "Could not find successor for address %#x" % state.addr
            )
        ...
        if len(succs) == 1:
            ...
            log.info(
                "Remaining states: "
                + ", ".join(
                    [str(key) + ": " + str(len(succs_dict[key])) for key in succs_dict]
                )
            )
    ...
    def _pick_correct_successor(self, succs):
        ...
        if not res:
            ...
            if trace_addr > prev_vex.addr and trace_addr < (
                prev_vex.addr + prev_vex.size
            ):
                ...
                if len(res) < 1:
                    log.error(
                        "Trace is in the middle of block %#x, cannot find %#x or %#x from %s"
                ...
                if rep_op == "cmpsb":
                    ...
                    if (
                        rep_con in ["repnz", "repne"]
                        and succs[-1].solver.eval(succs[-1].regs.al) == 0
                    ):
                        ...
                        log.warn(
                            "Encountered 'repnz scasb' that looks like it behaves like strlen, applying heuristic"
                        )
                        new_rcx = succs[-1].solver.BVS("scasb", 64)
                        con = succs[-1].solver.And(
                            new_rcx > 0xFFFFFFFFFFFFF000, new_rcx <= 0xFFFFFFFFFFFFFFFF
                        )
                    ...
                    raise AngrTracerError(
                        "Unhandled split caused by repeat: %s"
                        % prev_cap.insns[-1].mnemonic
                    )
            else:
                log.debug(succs[0].block(succs[0].history.bbl_addrs[-1]).capstone.insns)
                log.error(
                    "Looking for successor %#x, only have: %s"
                    % (trace_addr, ",".join([hex(succ.addr) for succ in succs]))
                )
                raise AngrTracerError("No states followed the trace?")

        if len(res) > 1:
            raise AngrTracerError(
                "The state split but several successors have the same (correct) address?"
            )
    ...
    def _update_state_tracking(self, state: "angr.SimState"):
        ...
        if state.history.recent_block_count > 1:
            ...
            assert state.history.recent_block_count == len(
                state.history.recent_bbl_addrs
            )
        ...
        if sync is not None:
            ...
            if self._trace[sync] == state.addr:
                ...
                raise Exception(
                    "Trace failed to synchronize! We expected it to hit %#x (untranslated), but it failed to do this within a timeout"
                    % self._trace[sync]
                )
            ...
            self.project.loader._extern_object is not None
            ...
            if proc is None:
                raise Exception(
                    "Extremely bad news: we're executing an unhooked address in the externs space"
                )
            ...
            state.history.jumpkind.startswith("Ijk_Exit")
            or state.globals["call_depth"] < 0
        ):
            # termination!
        ...
        if state.globals[
            "sync_idx"
        ] is not None and self.project.loader.find_object_containing(state.addr):
            log.info(
                "Trace: %d-%d/%d synchronizing %d %s",
                state.globals["trace_idx"],
                state.globals["sync_idx"],
                len(self._trace),
                state.globals["sync_timer"],
                self.project.loader.describe_addr(state.addr),
            )
        elif state.globals["sync_idx"] is not None:
            log.info(
                "Trace: %d-%d/%d synchronizing %d %#x",
                state.globals["trace_idx"],
                state.globals["sync_idx"],
                len(self._trace),
                state.globals["sync_timer"],
    ...
    def _analyze_misfollow(self, state, idx):
        ...
        log.info(
            "Misfollow: angr says %#x (%s), trace says %#x (%s)",
            angr_addr,
            ldr.describe_addr(angr_addr),
            trace_addr,
            ldr.describe_addr(trace_addr),
        )
        ...
        if len(curr_block.capstone.insns) > 0:
            mnemonics = [
                insn.mnemonic for insn in state.block(diverge_addr).capstone.insns
            ]
            if mnemonics[-1].startswith("rep"):
                try:
                    ...
                    raise AngrTracerError(
                        "At a disparate instruction, can't find where to sync to"
                    )
        ...
        if len(state.history.bbl_addrs) >= 2:
            prev_prev_addr = state.history.bbl_addrs[-2]
            if (
                state.block(prev_prev_addr).vex.jumpkind.startswith("Ijk_Sys")
                and self._trace[idx] == angr_addr
            ):
                state.globals["trace_idx"] = idx
                return True
    ...
    def _sync_callsite(self, state, idx, callsite_addr):
        ...
        try:
            ...
            log.error(
                "Trying to fix desync at callsite but return address does not appear in trace"
            )
            return False
    ...
    def _fast_forward(self, state):
        ...
        log.debug(
            "Searching for %#x starting at index %d"
            % (target_addr, state.globals["trace_idx"] + 1)
        )
        try:
            ...
            if not hook is None:
                msg += (
                    " Trace may have ended inside hooked function %s."
                    % hook.display_name
                )
