    ...
    if isinstance(param_ast, pycparser.c_ast.IdentifierType):
        ...
        if param_name in BASIC_TYPES:
            ...
            for child in file_ast.ext:
                if (
                    isinstance(child, pycparser.c_ast.Typedef)
                    and child.name == param_name
                ):
                    return parse_size(child, file_ast, size)
            raise Exception(
                "Failed to find declaration for %s, if it is a basic type, "
                "add it to the BASIC_TYPES dictionary." % param_name
            )

    elif isinstance(
        param_ast,
        (pycparser.c_ast.TypeDecl, pycparser.c_ast.Decl, pycparser.c_ast.Typedef),
    ):
        ...
        raise Exception(
            "Unhandled type in parse_size, erroring AST: %s" % str(param_ast)
        )
...
def parse_param(param_ast, file_ast, parent=None):
    ...
    if isinstance(
        param_ast, (pycparser.c_ast.IdentifierType, pycparser.c_ast.Constant)
    ):
        if isinstance(param_ast, pycparser.c_ast.IdentifierType):
            param_name = param_ast.names[0]
        else:  # constant
        ...
        if parent is None:
            raise Exception(
                "Prototype format does not support pass-by-value for Structs"
            )
        ...
        if param_ast.decls is None:
            print(
                "Warning: %s is an anonymous struct" % param_ast.name, file=sys.stderr
            )
        ...
        if param_ast.dim is None or not isinstance(
            param_ast.dim, pycparser.c_ast.Constant
        ):
            raise Exception("Cannot handle arrays of unknown size")
        if not param_ast.dim.type in BASIC_TYPES:
            raise Exception(
                "Array has an unsupported size type: %s" % param_ast.dim.type
            )

        array_size = BASIC_TYPES[param_ast.dim.type] * parse_size(
            param_ast.type, file_ast
        )
        ...
        if parent is None:
            raise Exception(
                "Reached FuncDecl inside a parameter, but no parent pointer?"
            )
        parent["value_type"] = "Ptr_Code"
        return
        ...
        raise Exception(
            "Unhandled type in parse_param, erroring AST: %s" % str(param_ast)
        )
...
def main():
    ...
    if len(sys.argv) < 4:
        print(
            "Usage: %s <c_header_file> <function_name> <output_directory> "
            "[cpp args...]" % os.path.basename(__file__),
            file=sys.stderr,
        )
    ...
    if not os.path.isdir(output_dir):
        print(
            "Error: Output directory does not exist: %s" % output_dir, file=sys.stderr
        )
        return EXIT_INVALID_ARGUMENT
    ...
    if os.path.exists(ofilepath):
        print(
            "Error: File already exists, refusing to overwrite: %s" % ofilepath,
            file=sys.stderr,
        )
    ...
    if not check_cpp():
        print(
            "Error: cpp does not appear to be installed, please install it. (on Debian, "
            "run: sudo apt install cpp)",
            file=sys.stderr,
        )
        return EXIT_RUNTIME_ERROR
    ...
    if not os.path.isdir(fake_libc):
        print(
            "Error: Cannot find required directory (should have been included with this "
            "program): %s" % fake_libc,
            file=sys.stderr,
        )
        return EXIT_RUNTIME_ERROR
    ...
    if func_ast is None:
        print(
            "Error: Failed to find function declaration: %s" % func_name,
            file=sys.stderr,
        )
        return EXIT_RUNTIME_ERROR
    ...
    for param_ast in func_params:
        if isinstance(param_ast, pycparser.c_ast.EllipsisParam):
            ...
            print(
                "Warning: Ellipsis function, only defining explicit parameters",
                file=sys.stderr,
            )
            continue
        ...
        try:
            ...
            print(
                "Error: Failed to parse function parameter: %s" % str(ex),
                file=sys.stderr,
            )
            return EXIT_RUNTIME_ERROR
